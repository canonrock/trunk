...
static int battle_calc_attack_skill_ratio(struct Damage wd, struct block_list *src,struct block_list *target,uint16 skill_id,uint16 skill_lv)
{
	...
	switch(skill_id) {
		...
		case WS_CARTTERMINATION:
			...
			if (sd && sd->cart_weight)
				skillratio += sd->cart_weight / i * 46000 / battle_config.max_cart_weight - 100;
			else if (!sd)
		...
		case RK_HUNDREDSPEAR:
			...
			if (sd) {
				...
				skillratio += 150 * pc_checkskill(sd,LK_SPIRALPIERCE);
...
#define MATK_ADD(a) { ad.damage += a; }
			...
			default: {
				...
				switch(skill_id) {
					...
					case SL_SMA: //Base damage is 40% + lv%
						skillratio += -80 + status_get_lv(src);
						break;
					case NJ_KOUENKA:
						skillratio -= 10;
		...
		switch(skill_id) {
			...
			case NPC_GRANDDARKNESS: {
					...
					ad.damage = battle_attr_fix(src, target, wd.damage + ad.damage, s_ele, tstatus->def_ele, tstatus->ele_lv) * (50 + 40 * skill_lv) / 100;
					if(src == target) {
						if(src->type == BL_PC)
	...
	switch (skill_id) {
		...
		case CR_ACIDDEMONSTRATION:
			...
			if(tstatus->vit+sstatus->int_) //crash fix
				md.damage = (int)((int64)50*tstatus->vit*sstatus->int_*sstatus->int_ / (30*(tstatus->vit+sstatus->int_)));
			else
				md.damage = 0;
			if (tsd) md.damage>>=1;
#endif
